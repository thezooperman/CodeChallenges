...
def batchDeleteMails(service):
    ...
    try:
        ...
        print(
            f'Total Messages: {len(to_delete_ids)}, Target Iterations: {iterations}')
        ...
        for i in range(iterations):
            ...
            take = len(to_delete_ids) if (take + batch_limit >=
                                          len(to_delete_ids)) else take + batch_limit
    ...
    if exception:
        ...
        try:
            ...
            while 'nextPageToken' in response:
                ...
                response = service.users().messages().list(userId=USER_ID, q=request_id,
...
if __name__ == '__main__':
    ...
    with open('mail_filter.txt', encoding='utf-8') as fp:
        mail_filter = [line.strip() for line in fp]
