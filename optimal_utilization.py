...
possible. Return a list of ids of selected elements.
...
Explanation:
There are only three combinations [1, 1], [2, 1], and [3, 1], which have a
total sum of 4, 6 and 8, respectively.
...
def closest_pairs(a: List[List[int]], b: List[List[int]], target: int) -> List[List[int]]:
    # TODO: basic checks
    
    # assuming inputs are sorted by key
    final_set = []
    min_diff = math.inf
...
if __name__ == "__main__":
    ...
    actual = closest_pairs(a, b, target)
    output = [[3, 1]]

    assert actual.sort() == output.sort(), print(actual)
