...
two numbers A[i] and B[j] such that:
    ...
    A[i] == B[j];
    The line we draw does not intersect any
...
Return the maximum number of connecting lines we can
draw in this way.
...
from A[1]=4 to B[2]=4 will intersect the line from A[2]=2 to B[1]=2.
        ...
        public int MaxUncrossedLines(int[] A, int[] B) 
        {
            ...
            for (int i = 1; i < size_a + 1; i++)
            {
                for (int j = 1; j < size_b + 1; j++)
                {
                    ...
                    {
                        // if items are not same, check 
                        // previous row, - 1 element, or
                        // previous column -1 element
