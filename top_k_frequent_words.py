...
highest to lowest. If two words have the same frequency,
...
class Solution:
    def topKFrequent(self, words: List[str], k: int) -> List[str]:
        ...
        for word in words:
            if word.strip() in frequency:
                frequency[word] += 1
            else:
                frequency[word] = 1

        frequency = list(
            sorted(frequency.items(), key=lambda x: (-x[1], x[0])))[:k]

        return [key[0] for key in frequency]


if __name__ == "__main__":
    obj = Solution()

    input_arr = ["i", "love", "leetcode", "i", "love", "coding"]
    k = 2
    output = ["i", "love"]

    actual = obj.topKFrequent(input_arr, k).sort()

    assert actual == output.sort()

    input_arr = ["the", "day", "is", "sunny", "the", "the", "the",
                 "sunny", "is", "is"]
    k = 4
    output = ["the", "is", "sunny", "day"]
